...
var page = Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        // console.log(data);
        rawMarks = data
        // 错误的 var t = util.marksFormat(data)
        // 必须为不同的对象
        that.setData({
          'marks': util.marksFormat(data),
          'marksBack': util.marksFormat(data),
        })
      }
    })
  ...
  regionchange(e) {
    ...
    myMap.getScale({
      success: function (res) {
        // 上次的缩放比例
        var oldScale = that.data.oldScale
        // 当前map缩放比例
        // var currentScale = (res.scale >= default_scale) ? default_scale : (res.scale).toFixed(9)
        var currentScale = res.scale.toFixed(9)
        ...
        if (oldScale == currentScale) {
          ...
          var marksBack = that.data.marksBack;
          // marks图标缩放
          var ratio = Math.pow(currentScale / default_scale, 10)
          //if (ratio == 0) ratio = 1
